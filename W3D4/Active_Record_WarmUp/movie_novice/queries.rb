...
def find_angelina
  ...
  # angelinas = Actor.where('first_name = ?', 'Angelina')
  # angelinas.where_values_hash
  #
  # angelina_jolie = angelinas.where('last_name = ?', 'Jolie')
  # angelina_jolie.where_values_hash
...
def uma_movies
  ...
  Movie.select(:id, :title, :yr).joins(:actors)
       .where('name = \'Uma Thurman\'').order('yr ASC')
end
